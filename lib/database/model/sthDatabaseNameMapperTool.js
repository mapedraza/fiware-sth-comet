 ...
 * @param  {Function} callback     The callback
 */
function copyDatabase2(options, databaseName, callback) {
...
function copyDatabase(options, databaseName, callback) {
    ...
    adminDB.db(sourceDB).command(listCommand, function onListCollections(err, results) {
        if (err) {
            return process.nextTick(callback.bind(null, err));
        }
        ...
        results.cursor.firstBatch.forEach((collection) => {
            ...
            let collectionData = { source: source, destination: destination, db: db };
            arrayCollectionRenames.push(collectionData);
        });

        async.map(arrayCollectionRenames, renameCollection, function(err, reslt) {
            if (err) {
                return process.nextTick(callback.bind(null, err));
            }
            sthLogger.info(
...
var renameCollection = function(element, callback) {
    element.db.command({ renameCollection: element.source, to: element.destination, dropTarget: true }, function(err) {
        if (err) {
            callback(1, element);
        } else {
